...
```{r, mean_total_steps_per_day, fig.width = 11}
...
The mean number of steps per day is `r day_mean`.  The median number of steps per day is `r day_median`.
...
```{r, average_daily_activity_pattern, fig.width = 11}
...
So we see that there are `r missing_rows` 
rows(`r round(missing_rows/nrow(data), digits = 2) * 100`%) with missing data.
...
```{r,imputed_daily_average, fig.width = 11}
...
Our new mean number of steps per day is `r day_mean_imp` and our new median number of
steps per day is `r day_median_imp`.  As you can see, our imputation strategy has 
...
```{r,weekend_weekday_activity_patterns, fig.width = 11}
...
`r int_max_wday$interval[1]`, than weekends, `r int_max_wday$interval[2]`.  This
